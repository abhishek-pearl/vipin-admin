    ...
    const getData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8000/api/v1/tagline/getTagline"
        );
  ...
  const handleAdd = async () => {
    ...
    if (heading.trim() && paragraph.trim()) {
      if (editingIndex !== null) {
        ...
        console.log(updatedItems[editingIndex], ",sdfsdfsdf");
        const res = await axios.put(
          `http://localhost:8000/api/v1/tagline/updateTagline/${updatedItems[editingIndex]._id}`,
          { heading, paragraph },
          {
            headers: {
        ...
        try {
          const res = await axios.post(
            "http://localhost:8000/api/v1/tagline/addTagline",
            { heading, paragraph },
            {
              headers: {
                "Content-Type": "application/json",
  ...
  const handleDelete = async (index, item) => {
    try {
      const res = await axios.delete(
        `http://localhost:8000/api/v1/tagline/deleteTagline/${item._id}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
      ...
      toast.success("fksfl;ksldfsldf");
    } catch (err) {
      console.log("Error in deleting Tagline", err);
    }
  };
